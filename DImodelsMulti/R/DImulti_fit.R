...
#'
#' @param y If the dataset is in a wide format, this argument is a vector of k column names identifying the ecosystem function values recorded
#' from each experimental unit in the dataset. For example, if the ecosystem function columns are labelled Y1 to Y4, then
#' \code{y = c("Y1","Y2","Y3","Y4")}. Alternatively, the column numbers can be specified, for example, \code{y = 8:11}, where ecosystem
#' function values are in the 8th to 11th columns. \cr If the dataset is in the long/stacked format, this argument is the column name of the
#' response value vector, for example, \code{y = "yield"}, alternatively, the column number can be supplied, \code{y = 8}
#' @param eco_func A vector of size two, with the first index holding the column name containing the character or factor indicator of which
#' response was recorded (for use when stacked/long data passed through parameter y), otherwise it is the string \code{"NA"}, case insensitive. The second index should contain a string referring to the autocorrelation structure of the responses (for use in multivariate data case), options include \code{"un"} for unstructured/general and \code{"cs"} for compound symmetry. For example, \code{eco_func = c("Function", "cs")}, pertaining to multivariate data in a stacked format with a compound symmetry structure, or \code{eco_func = c("na", "un")}, meaning either univariate or wide multivariate data with an unstructured/general variance covariance matrix
...
#' @param unit_IDs A vector of columns names/indices containing identifiers for the experimental units from which the observations
#' (either multiple readings of the same response or a single reading of multiple responses) are taken. Any number of identifiers can be
#' included but they must be in nested order, from largest to smallest, e.g., \code{unit_IDs = "Plot"} or
#' \code{unit_IDs = c("Plot", "ReplicateNo.")}
#' @param prop A vector of S column names identifying the species proportions in each community in the dataset. For example, if the species
#' proportions columns are labelled p1 to p4, then \code{prop = c("p1","p2","p3","p4")}. Alternatively, the column numbers can be specified,
#' for example, \code{prop = 4:7}, where species proportions are in the 4th to 7th columns
#' @param ID A text list (of length s) describing groupings for the identity of the effects of the species. For example, if there are four
#' species and you wish to have two identity effect groups where species 1 and 3 and species 2 and 4 are grouped together: ID could be
#' \code{ID = c("ID1", "ID2", "ID1", "ID2")}, where "ID1" and "ID2" are the names of the ID groups. This grouping does not affect the
#' interaction terms.
#' @param FG If species are classified by g functional groups, this argument takes a string vector (of length S) of the functional group to
#' which each species referenced in the \code{prop} argument belongs. For example, for four grassland species with two grasses and two legumes:
#' FG could be \code{FG = c("G","G","L","L")}, where G stands for grass and L stands for legume. This argument is optional but must be supplied
#' if the \code{"FG"} interaction structure is specified in the \code{what} argument
#' @param data A dataframe or tibble containing all previously input columns
#' @param extra_fixed A formula expression for any additional fixed effect terms. For example,  \code{extra_fixed = ~ Treatment} or
#' \code{extra_fixed = ~ Treatment + Density}
#' @param DImodel A string, referring to the interaction structure of model to be fit from the full list:
#' \code{"STR", "ID", "FULL", "E", "AV", "ADD", "FG"}
#' @param estimate_theta By default, \eqn{\theta} (the power parameter on all \code{p_i * p_j} components of each interaction variable
#' in the model) is set equal to one. Specify \code{estimate_theta = TRUE} to include the estimation of \eqn{\theta} in the specified model.
#' @param theta Users may specify a value of \eqn{\theta} different than 1 to fit the DI model. Note that if \code{estimate_theta = TRUE},
#' then \eqn{\theta} will be estimated via maximum profile log-likelihood and the value specified for \code{theta} will be overridden.
#' Specify a vector of positive non-zero numerical values indicating the value for the non-linear parameter of the model for each ecosystem
#' function (in alphabetical order, use \code{sort()} to find this) present in the dataset, or a single value to be used for all. For example,
#' \code{theta = 0.5} or \code{theta = c(1, 0.5, 1)}
#' @param method The string \code{"REML"} or \code{"ML"}, referring to the estimation method to be used
#'
#' @return \code{DImulti} - a custom class object containing the gls model fit with additional DI model attributes
#'
#' @details
#'
#' \cr
...
#' and/or time points from a BEF study.
#' Data should contain a column for each species' proportion, so that each row of these columns sum
#' to one.
#' Each row of the data should contain an identifier for the experimental unit being referenced,
#' these identifiers must be unique for
#' each unit.
...
#' & \eqn{j}.
#' The nonlinear term \eqn{\theta} (Connolly \emph{et al.}, 2013) is included in the model to allow
#' the shape of the BEF relationship to
#' change. This parameter can be estimated using profile log-likelihood optimisation (Brent, 1973)
#' or can be assigned a set value based on
#' an a priori assumption/knowledge.
#' \eqn{A} may include blocks or treatment terms, and \eqn{\alpha_{kt}} is a vector of the
#' corresponding effect coefficients.
#' \cr For further details of univariate DI modelling, see \code{?DImodels}, Kirwan \emph{et al.},
#' 2009, and Moral \emph{et al.}, 2023.
#' \cr \cr
#'
#' The multivariate DI model (Dooley \emph{et al.}, 2015) extends the DI modelling framework to
#' allow for the estimation of multiple
#' ecosystem functions simultaneously, accounting for any existing covariance between functions
#' through the error term. These models can
#' be further extended through the introduction of repeated measures, such as multiple time points.
#' \cr
...
#' \eqn{t}. For an experimental unit \eqn{m}, \eqn{\beta_{ikt}} scaled by \eqn{p_{i}} is the
#' expected contribution of the \eqn{i^{th}}
#' species to the \eqn{k^{th}} response at time point \eqn{t} and is referred to as the \eqn{i^{th}}
#' species’ ID effect.
...
#' species interactions could change between ecosystem functions. In the case that a dataset
#' contains only a single ecosystem function,
#' the corresponding subscript \eqn{k} can simply be removed from the equation, the same can be said
#' for the removal of the subscript \eqn{t}
#' in the instance that a dataset contains a single time point. \cr \cr
#'
#' \strong{The Structure of the Error Term} \cr \cr
#'
...
#' data, the error term is now assumed to
#' follow a multivariate normal distribution with mean \eqn{0} and variance \eqn{\Sigma^{*}}.
#' \deqn{ \epsilon \sim MVN(0, \Sigma^{*}) }
#' \eqn{\Sigma^{*}} is a block diagonal matrix, with one \eqn{kt} x \eqn{kt} block, \eqn{\Sigma},
#' for each experimental unit \eqn{m}. We refer
#' to \eqn{\Sigma} as the variance covariance matrix for our ecosystem functions and time points.
#' Typically, it includes a unique variance
#' per combination of ecosystem functions and time points along the diagonal and a unique covariance
#' between each pair of combinations on the
#' off-diagonal. Autocorrelation structures may be implemented on the matrix \eqn{\Sigma}, either to
#' simplify the estimation process or based
#' on a priori knowledge. One structure is chosen for the ecosystem functions and another for
#' repeated measures/time points, the two matrices
#' are then estimated independently and combined using the Kronecker product (\eqn{\otimes}), a
#' matrix multiplication method. In the case that
#' the data is only multivariate (\eqn{k>1} & \eqn{t=1}) or only has repeated measures (\eqn{k=1} &
#' \eqn{t>1}), only one autocorrelation structure
#' needs to be chosen, with no combination necessary.
#' Three such structures are currently available in this package: \cr
#' \enumerate{
...
#' 2023. Going beyond richness: Modelling the
#' BEF relationship using species identity, evenness, richness and species interactions via the
#' DImodels R package. Methods in Ecology and
#' Evolution, 14(9), pp.2250-2258. \cr \cr
#' Dooley, Á., Isbell, F., Kirwan, L., Connolly, J., Finn, J.A. and Brophy, C., 2015. Testing the
#' effects of diversity on ecosystem
#' multifunctionality using a multivariate model. Ecology Letters, 18(11), pp.1242-1251. \cr \cr
#' Finn, J.A., Kirwan, L., Connolly, J., Sebastià, M.T., Helgadottir, A., Baadshaug, O.H.,
#' Bélanger, G., Black, A., Brophy, C., Collins,
#' R.P. and Čop, J., 2013. Ecosystem function enhanced by combining four functional types of plant
#' species in intensively managed grassland
#' mixtures: a 3‐year continental‐scale field experiment. Journal of Applied Ecology, 50(2),
#'  pp.365-375 .\cr \cr
#' Connolly, J., Bell, T., Bolger, T., Brophy, C., Carnus, T., Finn, J.A., Kirwan, L., Isbell, F.,
#' Levine, J., Lüscher, A. and Picasso, V.,
#' 2013. An improved model to predict the effects of changing biodiversity levels on ecosystem
#' function. Journal of Ecology, 101(2), pp.344-355.
#' \cr \cr
#' Kirwan, L., Connolly, J., Finn, J.A., Brophy, C., Lüscher, A., Nyfeler, D. and
#' Sebastià, M.T., 2009. Diversity–interaction modeling:
#' estimating contributions of species identities and interactions to ecosystem function. Ecology,
#' 90(8), pp.2032-2038. \cr \cr
#' Brent, R.P., 1973. Some efficient algorithms for solving systems of nonlinear equations. SIAM
#' Journal on Numerical Analysis, 10(2),
#' pp.327-344. \cr \cr
#'
#' @seealso
#' This package:
...
#' # formally using a likelihood ratio test.
#'
#' anova(simModel_FULL, simModel_AV)
#'
#' # As the p-value is greater than our selected alpha value (0.05), we fail to reject the null
#' # hypothesis that our larger model is the better fit for our data, and so continue with
#' # simModel_AV. We can confirm this result by selecting the model with the lower AIC or BIC value.
#'
#' AIC(simModel_FULL); AIC(simModel_AV)
#' BIC(simModel_FULL); BIC(simModel_AV)
#'
...
#' head(dataSWE)
#'
#' # We transform the "YEARN" and "PLOT" columns to factors to better act as groups in our models.
#'
#' dataSWE$YEARN <- as.factor(dataSWE$YEARN)
#' dataSWE$PLOT <- as.factor(dataSWE$PLOT)
#'
...
#' # The interaction structure chosen is additive ("ADD").
#' # We include one of the treatment terms, TREAT, as an extra fixed effect crossed with each
#' # ID effect.
#' # We allow the value of theta to be estimated by the function and we opt to use the REML
...
#'                     unit_IDs = 2, DImodel = "ADD", extra_fixed = ~ TREAT*(G1 + G2 + L1 + L2),
#'                     estimate_theta = TRUE, method = "REML", data = dataSWE)
#' }
#' # The above code returns an error indicating that the model fit was singular, meaning that some
#' # elements of the variance covariance matrix were estimated to be zero. Fixing this usually
#' # involves simplifying the model.
#' # Normally, in order to improve the model we would fit different values of theta, different
#' # interaction structures, etc. and then formally compare the models, but for the sake of this
#' # example we will showcase one extreme example of how the above model can be simplified.
#' # We first remove any species interaction terms from the model by selecting the "ID" option for
#' # 'DImodel', which also removes the non-linear parameter theta.
#' # We then simplify the ID effects of the species by grouping the grasses and legumes through the
#' # 'ID' parameter. We also remove the treatment.
#'
#' SWEmodel <- DImulti(prop = 5:8, y = c("YIELD"), time = c("YEARN", "CS"),
#'                     unit_IDs = 2, DImodel = "ID", ID = c("G", "G", "L", "L"),
#'                     method = "REML", data = dataSWE)
#'
#' summary(SWEmodel, notes = FALSE)
#'
#' # From here we could begin slowly adding back information to the model and formally performing
#' # model selection using AIC, BIC, or likelihood ratio tests.
#'
#' @export
...
DImulti <- function(y, eco_func = c("NA", "NA"), time = c("NA", "NA"), unit_IDs,
                    prop, data, DImodel, ID = NULL, FG = NULL, extra_fixed = NULL,
                    estimate_theta = FALSE, theta = 1, method = "REML")
{
  ...
  {
    stop(crayon::bold(crayon::red("You must not have any columns named \"AV\" or \"E\", \"NA\" or \"NULL\" in your dataset provided through
                                  the parameter 'data'")))
  }
  if(any(startsWith(colnames(data), c("FULL.", "FG."))))
  {
    stop(crayon::bold(crayon::red("You must not have any column names beginning with \"FULL.\" or \"FG.\" in your dataset provided through the
                                  parameter 'data'")))
  }
  if(any(endsWith(colnames(data), c("_add"))))
  {
    stop(crayon::bold(crayon::red("You must not have any column names ending with \"_add\" in your dataset provided through the parameter
                                  'data'")))
  }
  #################
  ...
  {
    stop(crayon::bold(crayon::red("One or more of the columns referenced in the parameter 'prop' do not exist in the dataset specified through
                                  the 'data' parameter.\n")))
  }
  ...
  {
    warning(crayon::bold(crayon::red("One or more rows have species proportions that do not sum to 1. It is assumed that this is by design
                                     with the missing proportions specified through the parameter 'extraFixed', but please ensure this.\n")))
  }
  ...
  else if(any(pi_sums < 1 & pi_sums > 0.9999) | any(pi_sums > 1 & pi_sums < 1.0001))
  {
    ...
    warning(crayon::italic(crayon::red("One or more rows have species proportions that sum to approximately 1, but not exactly 1. This is
                                       typically a rounding issue, and has been corrected internally prior to analysis.\n")))
  }
  #####################
  ...
  {
    stop(crayon::bold(crayon::red("One or more of the columns referenced in the parameter 'y' do not exist in the dataset specified through
                                  the 'data' parameter.\n")))
  }
  if (!(ecoFunc[1] %in% colnames(data)) & toupper(ecoFunc[1]) != "NA")
  {
    stop(crayon::bold(crayon::red("The column referenced in the parameter 'eco_func' does not exist in the dataset specified through the
                                  'data' parameter.\n")))
  }
  if (!(time[1] %in% colnames(data)) & toupper(time[1]) != "NA")
  {
    stop(crayon::bold(crayon::red("The column referenced in the parameter 'time' does not exist in the dataset specified through the 'data'
                                  parameter.\n")))
  }
  ...
  {
    stop(crayon::bold(crayon::red("You must either enter the column name for your ecosystem function response indicator as a string or \"NA\"
                                  for the first position in the vector parameter 'eco_func'")))
  }
  if (!is.character(time[1]))
  {
    stop(crayon::bold(crayon::red("You must either enter the column name for your time indicator as a string or \"NA\" for the first position
                                  in the vector parameter 'time'")))
  }
  ...
  {
    stop(crayon::bold(crayon::red("You specified that data is both long and wide. You must supply either multiple response variable names or
                                  column indices through the argument 'y' (wide data) OR a single response value column through the parameter
                                  'y' and a single response indicator column through the parameter 'eco_func' (long/stacked data)\n")))
  }

  if(length(y) < 2 & toupper(ecoFunc[1]) == "NA" & toupper(time[1]) == "NA")
  {
    stop(crayon::bold(crayon::red("You must supply either multiple response variable names or column indices through the argument 'y' (wide
                                  data), a single response value column through the parameter 'y' and a single character/factor response
                                  indicator column through the parameter 'eco_func' (long/stacked data), or a repeated measure indicator
                                  column through the 'time' parameter.\n")))
  }

  if(toupper(ecoFunc[2]) != "NA" & length(y) < 2 & toupper(ecoFunc[1]) == "NA")
  {
    warning(crayon::bold(crayon::red("You supplied a response correlation structure with only a single response, the correlation structure
                                     will be ignored.\n")))
  }

  if(toupper(time[2]) != "NA" & toupper(time[1]) == "NA")
  {
    warning(crayon::bold(crayon::red("You supplied a time correlation structure with no repeated measure specified, the correlation structure
                                     will be ignored.\n")))
  }
  #################
  ...
  {
    stop(crayon::bold(crayon::red("One or more of the columns referenced in the parameter 'unit_IDs' do not exist in the dataset specified
  ...
  {
    warning(crayon::bold(crayon::red("You have supplied values for theta while estimate_theta is set to TRUE, theta will be estimated and the
                                     theta values given will be overridden")))
  }
  ...
  else if(Stackedflag)
  {
    ...
    {
      stop("You must suppy a value of theta for each ecosystem function (ecoFunc), or a single value of theta to use for all ecosystem
           functions")
    }
  }
...
## Interaction Columns & Theta ############################################################################################################################################################
  ...
  if(DImodel != "STR" & DImodel != "ID") # Interactions?
  {
    ...
    if(estimateTheta)
    {
      ...
      {
        tempModel <- DImodels::DI(y = Yvalue, prop = prop, FG = FG, DImodel = DImodel, extra_formula = extraFixed, data = data,
                                  estimate_theta = TRUE)
      ...
      else #split by ecosystem function
      {
        ...
        {
          tempModel <- DImodels::DI(y = Yvalue, prop = prop, FG = FG, DImodel = DImodel, extra_formula = extraTemp,
  ...
  {
    warning(crayon::italic(crayon::red("You specified for theta to be estimated or supplied values for theta but selected a DI mdoel type
                                       with no interactions, this will be ignored.")))
  }
  ...
  else if(FULLflag)
  {
    ...
    #Call fit
    fitReturn <- fit_FULL(formulaStart, formulaEnd, prop, ID, weightGen, corr, method, data)
    formula <- fitReturn[[1]]
    model <- fitReturn[[2]]
  ...
  else if(Eflag)
  {
    ...
    #Call fit
    fitReturn <- fit_E(formulaStart, formulaEnd, prop, ID, weightGen, corr, method, data)
    formula <- fitReturn[[1]]
    model <- fitReturn[[2]]
  ...
  else if(AVflag)
  {
    ...
    #Call fit
    fitReturn <- fit_AV(formulaStart, formulaEnd, prop, ID, weightGen, corr, method, data)
    formula <- fitReturn[[1]]
    model <- fitReturn[[2]]
  ...
  else if(ADDflag)
  {
    ...
    #Call fit
    fitReturn <- fit_ADD(formulaStart, formulaEnd, prop, ID, weightGen, corr, method, data)
    formula <- fitReturn[[1]]
    model <- fitReturn[[2]]
  ...
  else if(FGflag)
  {
    ...
    #Call fit
    fitReturn <- fit_FG(formulaStart, formulaEnd, prop, ID, weightGen, corr, method, data)
    formula <- fitReturn[[1]]
    model <- fitReturn[[2]]
  ...
  if(name != "Intercept Only Model" & name != "ID Model")
  {
    ...
    cat(crayon::bold(crayon::underline(round(theta, 4))), sep = ", ")
  }
