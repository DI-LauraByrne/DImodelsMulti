...
DI_vcov <- function(model, MVflag, Timeflag, funcCorr = "NA", timeCorr = "NA", formulaStart, formulaEnd, prop, ID, Yfunc, Yvalue, unitIDs, method, timeCol = "NA", data)
{
  corrMV <- corrT <- NULL
      ...
      corr <- switch(
        ...
        "un@un" = {
          ...
          models <- DImulti_vcovModels(model, MVflag, funcCorr = list(un, "un"), timeCorr = list(un, "un"), formulaStart, formulaEnd, unitIDs,
                                       prop, ID, Yfunc, Yvalue, method, timeCol, data)
          MVmodel <- models[[1]][[1]]
          Tmodel <- models[[2]][[1]]
        ...
        "un@ar1" = {
          ...
          models <- DImulti_vcovModels(model, MVflag, funcCorr = list(un, "un"), timeCorr = list(ar1, "ar1"), formulaStart, formulaEnd,
                                       unitIDs, prop, ID, Yfunc, Yvalue, method, timeCol, data)
          MVmodel <- models[[1]][[1]]
          Tmodel <- models[[2]][[1]]
                               ...
                               timeCol, data)
{
  ...
  switch(
    ...
    "FULL" = list(
      fit_FULL(formulaStartMV, formulaEndMV, prop, ID, weightMV, funcCorr, method, dataMV)[[2]],
      fit_FULL(formulaStartT, formulaEndT, prop, ID, weightT, timeCorr, method, dataT)[[2]]
    ),
    "E" = list(
      fit_E(formulaStartMV, formulaEndMV, prop, ID, weightMV, funcCorr, method, dataMV)[[2]],
      fit_E(formulaStartT, formulaEndT, prop, ID, weightT, timeCorr, method, dataT)[[2]]
    ),
    "AV" = list(
      fit_AV(formulaStartMV, formulaEndMV, prop, ID, weightMV, funcCorr, method, dataMV)[[2]],
      fit_AV(formulaStartT, formulaEndT, prop, ID, weightT, timeCorr, method, dataT)[[2]]
    ),
    "ADD" = list(
      fit_ADD(formulaStartMV, formulaEndMV, prop, ID, weightMV, funcCorr, method, dataMV)[[2]],
      fit_ADD(formulaStartT, formulaEndT, prop, ID, weightT, timeCorr, method, dataT)[[2]]
    ),
    "FG" = list(
      fit_FG(formulaStartMV, formulaEndMV, prop, ID, weightMV, funcCorr, method, dataMV)[[2]],
      fit_FG(formulaStartT, formulaEndT, prop, ID, weightT, timeCorr, method, dataT)[[2]]
    ),
  )
}
